...
def histogram_equalize(im_orig: np.ndarray) -> List:
    ...
    # TODO: Make sure you stretch the equalized histogram if it only covers partly that range.
    # np.where(cum_hist == 0)[0][0]
    # np.where(cum_hist == cum_hist.max())[0][-1]
    ...
    # TODO: try another way to calc hist_eq: newh[T(i)] += h(i)
    im_eq = cum_hist[np.multiply(im_orig, 255).astype(np.uint8)]
    # print(im_orig.dtype, im_orig.max())
...
if __name__ == '__main__':
    ...
    print(f'{np.argmax(a == 1)}   {np.amin(a == 1)}')
